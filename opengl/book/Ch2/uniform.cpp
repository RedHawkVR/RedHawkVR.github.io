/* Initialising uniform variables in a named uniform block */
...
void init() {
	...
	// Initialize uniform values in unform block "Uniforms"
	GLuint uboIndex;
	GLint uboSize;
	GLuint ubo;
	GLvoid *buffer;
	...
	if(buffer != NULL) {
		...
		// Copy uniform values into the buffer
		...
		// Create the uniform buffer object, initialize its storage, and associate it with the shader program
		glGenBuffers(1, &ubo);
